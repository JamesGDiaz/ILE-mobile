...
export default class ClinicMapScreen extends React.Component {
  ...
  populateScrollView() {
    return this.state.providers.map((provider, index) => (
      <View style={styles.option} key={provider.key}>
        ...
        <Switch
          style={{ flex: 0.4, marginRight: 20, marginLeft: 30 }}
          onValueChange={() => (
            (Providers[index].markersAreVisible = !Providers[index]
              .markersAreVisible),
  ...
  populateMarkers(provider) {
    if (provider.markersAreVisible == true) {
      return provider.markers.map(marker => (
        <Marker
          key={uuid.v4()}
    ...
    return (
      <View style={styles.containerView}>
        <MapView
          ...
          loadingEnabled={true}
        >
          {this.state.providers.map(provider => this.populateMarkers(provider))}
        </MapView>
        <View
          style={{
            flex: 0.15,
